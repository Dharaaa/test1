...
public class HallArrangementExamHallNoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edt_search.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                Call<HallArrangementStudentInformationPojo> call = apiInterface.FillExamHallNoList(entity_id, branch_id,edt_search.getText().toString());
                    ...
                    public void onResponse(Call<HallArrangementStudentInformationPojo> call, Response<HallArrangementStudentInformationPojo> response) {
                        ...
                        if (StatusCode.equals("200")) {
                            ...
                            for (int i = 1; i < Size; i++) {
                                ...
                                pojo.setROOMMASTERID(pojoitem.getHallArrangementStudentInformation().get(i).getROOMMASTERID());
                                pojo.setrOOMMASTERNAME(pojoitem.getHallArrangementStudentInformation().get(i).getrOOMMASTERNAME());


                                arrayList.add(pojo);

                            }
                            ExamHallNoAdapter adapter = new ExamHallNoAdapter(HallArrangementExamHallNoActivity.this, arrayList);
                            ...
                            mProgressDialog.dismiss();
                            Toast.makeText(HallArrangementExamHallNoActivity.this, DisplayMsg, Toast.LENGTH_SHORT).show();
                        }
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                 if(appController.getHallArrangementExamNoFlag().equals("HallArrangement")){
                    appController.setHall_arrangement_hallNo(String.valueOf(arrayList.get(position).getROOMMASTERID()));
                    appController.setHall_arrangement_HallName(arrayList.get(position).getrOOMMASTERNAME());

                    finish();
                }else if(appController.getHallArrangementExamNoFlag().equals("StudentList")){
                     appController.setStudent_list_hallId(String.valueOf(arrayList.get(position).getROOMMASTERID()));
                     appController.setStudent_list_hallName(arrayList.get(position).getrOOMMASTERNAME());
...
if(error!=null) {
    Toast.makeText(HallArrangementExamHallNoActivity.this, error, Toast.LENGTH_SHORT).show();
}else{

    if (mProgressDialog == null)
        mProgressDialog = new TransparentProgressDialog(HallArrangementExamHallNoActivity.this);
    ...
    Call<HallArrangementStudentInformationPojo> call = apiInterface.FillExamHallNoList(entity_id, branch_id,"");
    call.enqueue(new Callback<HallArrangementStudentInformationPojo>() {
        @Override
        public void onResponse(Call<HallArrangementStudentInformationPojo> call, Response<HallArrangementStudentInformationPojo> response) {
            Log.e("Response batch ", new Gson().toJson(response.body()));
...
Log.e("Room Master ",pojoitem.getHallArrangementStudentInformation().get(i).getrOOMMASTERNAME() + "**");
                    arrayList.add(pojo);
