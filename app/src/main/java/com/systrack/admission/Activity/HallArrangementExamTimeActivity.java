...
import com.systrack.admission.adapter.ExaminationAdapter.ExamHallNoAdapter;
import com.systrack.admission.adapter.ExaminationAdapter.ExamTimeAdapter;
...
public class HallArrangementExamTimeActivity extends AppCompatActivity {
    ...
    String hallId,examId,year,error;
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     if(appController.getHallArrangementExamNameFlag().equals("HallArrangement")){
         ...
         }else if(appController.getHall_arrangement_HallName()==null){
             error="Exam Hall Required";
         }else if(appController.getHall_arrangement_yearId()==null){
             error="Academic Year Required";
         }else{
             hallId=appController.getHall_arrangement_hallNo();
             examId=appController.getHall_arrangement_examId();
             year=appController.getHall_arrangement_yearId();

         }
     }else if(appController.getHallArrangementExamNameFlag().equals("StudentList")){
         if(appController.getStudent_list_examId()==null){
             error="Exam Name Required";
         }else if(appController.getStudent_list_hallId()==null){
             error="Exam Hall Required";
         }else if(appController.getStudent_list_year_id()==null){
             error="Academic Year Required";
         }else{
             hallId=appController.getStudent_list_hallId();
             examId=appController.getStudent_list_examId();
             year=appController.getStudent_list_year_id();
        ...
        edt_search.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                Call<HallArrangementStudentInformationPojo> call = apiInterface.FillExamTimeList(entity_id,
                        branch_id,year,examId,
                         hallId,edt_search.getText().toString());
                call.enqueue(new Callback<HallArrangementStudentInformationPojo>() {
                    ...
                    public void onResponse(Call<HallArrangementStudentInformationPojo> call, Response<HallArrangementStudentInformationPojo> response) {
                        ...
                        if (StatusCode.equals("200")) {
                            ...
                            ExamTimeAdapter adapter = new ExamTimeAdapter(HallArrangementExamTimeActivity.this, arrayList);
...
//                            linear_Search.setVisibility(View.GONE);
                            ...
                            mProgressDialog.dismiss();
                            Toast.makeText(HallArrangementExamTimeActivity.this, DisplayMsg, Toast.LENGTH_SHORT).show();
                        }
...
//                 if(appController.getClassTimingBatchFlag().equals("HallArrangement")){
                    appController.setStudent_list_examTimeName(arrayList.get(position).getdISPLAYEXAMTIME());
...
if(error!=null) {
    Toast.makeText(HallArrangementExamTimeActivity.this, error, Toast.LENGTH_SHORT).show();
}else{

    if (mProgressDialog == null)
        mProgressDialog = new TransparentProgressDialog(HallArrangementExamTimeActivity.this);
    ...
    Call<HallArrangementStudentInformationPojo> call = apiInterface.FillExamTimeList(entity_id, branch_id
            ,year,examId,hallId,"");
    call.enqueue(new Callback<HallArrangementStudentInformationPojo>() {
        @Override
        public void onResponse(Call<HallArrangementStudentInformationPojo> call, Response<HallArrangementStudentInformationPojo> response) {
